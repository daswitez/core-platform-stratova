...
public class AssignPermissionsToRoleController {
    ...
    private static final Logger log = LoggerFactory.getLogger(AssignPermissionsToRoleController.class);

    private final AssignPermissionsToRoleOrchestrator assignPermissionsToRoleOrchestrator;

    public AssignPermissionsToRoleController(AssignPermissionsToRoleOrchestrator assignPermissionsToRoleOrchestrator) {
    ...
    @Operation(
            ...
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    ...
                    content = @Content(
                            ...
                            schema = @Schema(implementation = PermissionIdsRequest.class),
    ...
    @ApiResponses(value = {
            @ApiResponse(
                    ...
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = AssignPermissionsToRoleResponse.class),
                            examples = @ExampleObject(
    ...
    public ResponseEntity<AssignPermissionsToRoleResponse> assignPermissionsToRole(
            ...
            @RequestBody
            @Valid
            @Schema(description = "List of permission IDs to assign to the role", required = true)
            PermissionIdsRequest request
    ) {
        log.debug("event=IAM_ASSIGN_PERMISSIONS_HTTP_REQUEST_RECEIVED roleId={} permissionIdsCount={}",
                roleId, request.permissionIds() != null ? request.permissionIds().size() : 0);

        AssignPermissionsToRoleRequest orchestratorRequest = new AssignPermissionsToRoleRequest(
                roleId,
                request.permissionIds()
        );

        AssignPermissionsToRoleResponse response = assignPermissionsToRoleOrchestrator.execute(orchestratorRequest);
